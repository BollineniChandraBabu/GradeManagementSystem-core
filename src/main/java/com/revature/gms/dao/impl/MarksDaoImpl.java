...
public class MarksDaoImpl implements MarksDao {
	...
	public List<Marks> getMarksById(int id) {
		...
		try {
			...
			String sql = "select m.id,m.sid,m.subid,m.marks,s.id,s.name,st.regno,st.name from marks as m, subjects as s,students as st where st.regno=m.sid and m.subid=s.id and st.regno=? order by m.subid asc";
	...
	public List<Marks> viewAllMarks() {
		...
		try {
			...
			connection = DbUtils.getConnection();
			String sql = "select m.id,m.sid,m.subid,m.marks,s.id,s.name,st.regno,st.name from marks as m,students as st,subjects as s where m.subid=s.id and st.regno=m.sid order by st.regno,s.id asc";
			preparedStatement = connection.prepareStatement(sql);
			resultSet = preparedStatement.executeQuery();
			while(resultSet.next()) {
				if(id==0) 
